...
namespace Assets.Scripts.Stages.FifthStage
{
    public class FifthStageExam : MonoBehaviour
    {
        ...
        public void AddFourthStageExamAddition(string name, string action, string idealAction, int right, int wrong)
        {
        ...
        private void CheckConfigureSettings()
        {
            if (ConfiguredSAT)
            {
                AddFourthStageExam("Настройка соединения в конфигураторе счетчика", "Правильно", "Задать параметры соединения (нечет), выбрать порт", 4, 0);
            }
            else
            {
                AddFourthStageExam("Настройка соединения в конфигураторе счетчика", "Неправильно", "Задать параметры соединения (нечет), выбрать порт", 0, 0);
            }
        }
        ...
        {
            if(_sumPoints.Point1 == 8)
            {
                AddFourthStageExam("Заведение точки УСПД в «Пирамиде»", "Правильно", "Ввести серийный номер УСПД, данные в полях «Пользователь» и «Пароль», выбрать часовой пояс, указать место установки. Настроить маршрут, ввести адрес и порт", _sumPoints.Point1, 0);
            }
            else
            {
                AddFourthStageExam("Заведение точки УСПД в «Пирамиде»", "Неправильно", "Ввести серийный номер УСПД, данные в полях «Пользователь» и «Пароль», выбрать часовой пояс, указать место установки. Настроить маршрут, ввести адрес и порт", _sumPoints.Point1, 0);
            }
            CreateUSPDPointAddition();
        }
        private void CreateUSPDPointAddition()
        {
            string[] Action = new string[8] { "Добавить каналообразующее оборудование", 
                "Указать серийный номер оборудования", 
                "Заполнить поля «Пользователь» и «Пароль»", 
            ...
            for (int i = 0; i < Action.Length; i++)
            {
                AddFourthStageExamAddition(Action[i], _sumPoints.ReportPoint1[i], "", 0, 0);
            }
        }
        private void SettingPointAbonent()
        {
            if(_sumPoints.Point2 == 12)
            {
                ...
                AddFourthStageExam("Настройка точки учета абонента", "Правильно", "Выбрать счетчик в программе, указать его серийный номер, дату выпуска, установки, последней и следующей поверки, часовой пояс и связной номер. Настроить маршрут. Привязать счетчик к точке учета", _sumPoints.Point2, 0);
            }
            else
            {
                AddFourthStageExam("Настройка точки учета абонента", "Неправильно", "Выбрать счетчик в программе, указать его серийный номер, дату выпуска, установки, последней и следующей поверки, часовой пояс и связной номер. Настроить маршрут. Привязать счетчик к точке учета", _sumPoints.Point2, 0);
            }
        ...
        private void SettingDataCollection()
        {
            if (_sumPoints.Point3 == 13)
            {
                ...
                AddFourthStageExam("Настройка сценария сбора данных", "Правильно", "Создать сценарий сбора данных: указать наименование, глубину сбора, параметры, время принудительного завершения работы, включить проверку наличия данных в БД перед сбором, чтений событий, запуск сценария сразу после старта сервиса.\r\nНастроить расписание для сценария сбора данных: тип, периодичность выполнения, начало работы", _sumPoints.Point3, 0);
            }
            else
            {
                AddFourthStageExam("Настройка сценария сбора данных", "Неправильно", "Создать сценарий сбора данных: указать наименование, глубину сбора, параметры, время принудительного завершения работы, включить проверку наличия данных в БД перед сбором, чтений событий, запуск сценария сразу после старта сервиса.\r\nНастроить расписание для сценария сбора данных: тип, периодичность выполнения, начало работы", _sumPoints.Point3, 0);
            }
            SettingDataCollectionAddition();
        }
        ...
        {
            if (_sumPoints.Point4 == 6)
            {
                AddFourthStageExam("Создание абонента в базе", "Правильно", "Ввести данные абонента, выбрать тип абонента, указать счетчик", _sumPoints.Point4, 0);
            }
            else
            {
                AddFourthStageExam("Создание абонента в базе", "Неправильно", "Ввести данные абонента, выбрать тип абонента, указать счетчик", _sumPoints.Point4, 0);
            }
        ...
        private void DataForSingIn()
        {
            if(_sumPoints.Point5 == 3)
            {
                ...
                AddFourthStageExam("Задание данных для входа физлица в личный кабинет «Пирамиды»", "Правильно", "Выбрать абонента, указать тип профиля и аутентификации, задать пароль для пользователя", _sumPoints.Point5, 0);
            }
            else
            {
                AddFourthStageExam("Задание данных для входа физлица в личный кабинет «Пирамиды»", "Неправильно", "Выбрать абонента, указать тип профиля и аутентификации, задать пароль для пользователя", _sumPoints.Point5, 0);
            }
            DataForSingInAddition();
        }
        ...
        {
            if(_sumPoints.Point6 == 6 && _sumPoints.IsHaveIndications)
            {
                ...
                AddFourthStageExam("Получение показаний со счетчика", "Правильно", "Выбрать счетчик, задать интервал, выбрать параметр измерения", _sumPoints.Point6, 0);
            }
            else if(_sumPoints.IsHaveIndications)
            {
                AddFourthStageExam("Получение показаний со счетчика", "Неправильно", "Выбрать счетчик, задать интервал, выбрать параметр измерения", _sumPoints.Point6, 0);
            }
            else
            {
                AddFourthStageExam("Получение показаний со счетчика", "Неправильно", "Выбрать счетчик, задать интервал, выбрать параметр измерения", 0, 0);
            }
            GetInfoCounterAddition();
        }
