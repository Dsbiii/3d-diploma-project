...
public class SelectDrop : MonoBehaviour
{
    ...
    void Start()
    {
        try
        {
            ...
            if (_Type == Type.CalculatedSumPower)
            {
                ...
                GetComponent<Dropdown>().options[1].text = "0,48кВт";
                GetComponent<Dropdown>().options[2].text = "0,68кВт";
                GetComponent<Dropdown>().options[3].text = w.ToString() + "кВт";
    ...
    {
        // Получаем выбранный текст из Dropdown
